     ...
     if(firebaseUser){
       firebase.database().ref("la-presentacion-el-paraiso/config").on("value", snap => {
         ...
         if(config_2 == "active"){
           ...
           //Room On Click is into this function
           bringGroups()
}else{
...
function bringGroups(roc){
  firebase.database().ref("la-presentacion-el-paraiso/groups/").once("value", snap => {
    var groups = Object.keys(snap.val())
    groups.forEach(g => {
      let html = `<div class="room" id="room-${g}"><a href="#${g}">${g}</a>°</div>`
      $(".con-rooms").append(html)
    })
    var room = $(".room a")
    roomOnClick(room, userId)
  })
...
function roomOnClick(room, userId){
